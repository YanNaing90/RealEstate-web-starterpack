import CountUp from 'react-countup';
import './Hero.css';
import {HiLocationMarker} from 'react-icons/hi';
import { motion } from 'framer-motion';

const Hero = () => {
  return (
    <section className='hero-wrapper'>
        <div class="paddings innerWidth flexCenter hero-container">

            {/* Left Side */}
            <div class="flexColStart hero-left">
                <div class="hero-title">
                    <div class="orange-circle" />
                    <motion.h1 initial={{y:"2rem", opacity:0}} animate={{y:0,opacity:1}} 
                    transition={{duration:2, type:"spring"}}>
                        Discover <br/> Most Suitable <br/> Property
                    </motion.h1>
                    <h3>@Myanmar</h3>
                </div>
                <div class="flexColStart hero-des">
                    <span className='secondaryText'>Find a variety of property that suit you every easilty.</span>
                    <span className='secondaryText'>In Myanmar, forget all difficulties in finding a residence for you.</span>
                </div>
                <div class="flexCenter search-bar">
                    <HiLocationMarker color="var(--blue)" size={25} />
                    <input type='text' />
                    <button className='button'>Search</button>
                </div>
                <div class="flexCenter stats">
                    <div class="flexColCenter stat">
                        <span>
                            <CountUp start={8800} end={9000} duration={4}/>
                            <span>+</span>
                        </span>
                        <span className='secondaryText'>Premium Products</span>
                    </div>
                    <div class="flexColCenter stat">
                        <span>
                            <CountUp start={1950} end={2000} duration={4}/>
                            <span>+</span>
                        </span>
                        <span className='secondaryText'>Happy Customers</span>
                    </div>
                    <div class="flexColCenter stat">
                        <span>
                            <CountUp end={28} />
                            <span>+</span>
                        </span>
                        <span className='secondaryText'>
                            Award Winning
                        </span>
                    </div>
                </div>
            </div>
            <div class="flexCenter hero-right">
                <motion.div initial={{x:"7rem", opacity:0}} 
                animate={{x:0, opacity:1}} transition={{duration:2, type:"spring"}} class="image-container">
                    <img src='./hero-image.png' alt='' />
                </motion.div>
            </div>
        </div>
    </section>
  )
}

export default Hero